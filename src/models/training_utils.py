...
from src.features.build_features import get_dummy_values, fill_missing_values
...
def hide_test_data_ratings_for_prediction(Xr_test, X_test_raw, perc_test=0.3, min_ratings=3,
    ...
    for uid in selected_uids:
        ...
        X_test_raw_pred_hidden.loc[
            (X_test_raw_pred_hidden.user_id == uid) & (X_test_raw_pred_hidden.item_id == item_id)] = 0
...
def train_validation_test_split(
    ...
    Xf_train = X_train_raw.drop([USER_ID_COL, ITEM_ID_COL, RATING], axis=1).to_numpy()

    Xr_test = prepare_sparse_ratings_matrix(X_test_raw, nuser, nitem)
    Xf_test = X_test_raw.drop([USER_ID_COL, ITEM_ID_COL, RATING], axis=1).to_numpy()

    Xr_val = prepare_sparse_ratings_matrix(X_val_raw, nuser, nitem)
    Xf_val = X_val_raw.drop([USER_ID_COL, ITEM_ID_COL, RATING], axis=1).to_numpy()
